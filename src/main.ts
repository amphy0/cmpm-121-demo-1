...
const availableItems: Item[] = [
    { name: "Home Sushi Chef", cost: 10, rate: 0.1, quantity: 0 },
    { name: "Amateur Sushi Chef", cost: 100, rate: 2, quantity: 0 },
    { name: "Pro Sushi Chef", cost: 1000, rate: 50, quantity: 0 }
];
...
const itemElements: { button: HTMLButtonElement; label: HTMLParagraphElement }[] = availableItems.map((item) => {
...
availableItems.forEach((item, index) => {
    itemElements[index].button.addEventListener("click", () => {
        if (counter >= item.cost) {
            ...
            item.cost *= 1.15;
            item.quantity += 1;
            itemElements[index].button.textContent = `Purchase ${item.name} (${item.cost.toFixed(0)}🍣 each)`;
            updateUI();
        }
    });
});
